...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/debug/organizations", requireAuth, requireRole(["hostel_admin"]), async (req, res) => {
    try {
  ...
  app.post("/api/debug/run-migration", requireAuth, requireRole(["hostel_admin"]), async (req, res) => {
    try {
  ...
  app.post("/api/debug/create-test-org", requireAuth, requireRole(["hostel_admin"]), async (req, res) => {
    try {
  ...
  app.get("/api/orders/since", requireAuth, async (req, res) => {
    try {
      ...
      if (user.role === 'cafe_manager' || user.role === 'hostel_admin') {
  ...
  app.get("/api/realtime/stats", requireAuth, requireRole(["hostel_admin"]), (req, res) => {
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    try {
      ...
      // Find user by reset token (also checks if token is expired)
  ...
  // Verify reset token (to check if link is valid before showing reset form)
  ...
  app.get("/api/admin/menu/items", requireAuth, requireRole(["hostel_admin", "hostel_team", "cafe_manager"]), async (req, res) => {
    try {
  ...
  app.get("/api/admin/menu/categories", requireAuth, requireRole(["hostel_admin", "hostel_team", "cafe_manager"]), async (req, res) => {
    try {
  ...
  app.get("/api/rooms/available", requireAuth, async (req, res) => {
    try {
      const { site } = req.query;
      const rooms = await storage.getCommonRooms(site as string);
  ...
  app.post("/api/menu/items", requireAuth, requireRole(["hostel_admin", "hostel_team", "cafe_manager"]), async (req, res) => {
    try {
  ...
  app.patch("/api/menu/items/:id", requireAuth, requireRole(["hostel_admin", "hostel_team", "cafe_manager"]), async (req, res) => {
    try {
  ...
  app.delete("/api/menu/items/:id", requireAuth, requireRole(["hostel_admin", "hostel_team", "cafe_manager"]), async (req, res) => {
  ...
  app.delete("/api/menu/items/:id/force", requireAuth, requireRole(["hostel_admin", "hostel_team", "cafe_manager"]), async (req, res) => {
    const id = parseInt(req.params.id);
    try {
  ...
  app.get("/api/cafe/orders/all", requireAuth, requireRole(["cafe_manager", "hostel_admin"]), async (req, res) => {
    try {
      const user = req.user as schema.User;
      const { site } = req.query;
      
      // For cafe managers, use their site. For admins, use the site query parameter
      const filterSite = user.role === 'hostel_admin' ? (site as string) : user.site;
  ...
  app.patch("/api/cafe/orders/:id/status", requireAuth, requireRole(["cafe_manager", "hostel_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.patch("/api/cafe/orders/:id/payment", requireAuth, requireRole(["cafe_manager", "hostel_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.post("/api/cafe/orders/create-on-behalf", requireAuth, requireRole(["cafe_manager", "hostel_admin"]), async (req, res) => {
    try {
  ...
  app.get("/api/cafe/users", requireAuth, requireRole(["cafe_manager", "hostel_admin"]), async (req, res) => {
    try {
      const user = req.user as schema.User;
      const { site } = req.query;
      ...
      .where(
        and(
          ...
          or(
            ...
            inArray(schema.users.role, ["hostel_admin", "hostel_team"] as any)
          )
        )
      )
  ...
  app.get("/api/rooms/:id/bookings", requireAuth, async (req, res) => {
    try {
      ...
      const bookings = await db.select()
        ...
        .where(
          sql`${schema.common_room_bookings.room_id} = ${roomId} AND ${schema.common_room_bookings.status} = 'confirmed' AND DATE(${schema.common_room_bookings.start_time}) = ${date}`
        )
        .orderBy(asc(schema.common_room_bookings.start_time));
  ...
  app.post("/api/rooms", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const result = schema.insertCommonRoomSchema.safeParse(req.body);
      if (!result.success) {
        return res.status(400).json({ message: "Invalid input", errors: result.error.issues });
      }

      const room = await storage.createCommonRoom(result.data);
  ...
  app.patch("/api/rooms/:id", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const updates = req.body;
      
      const room = await storage.updateCommonRoom(id, updates);
      res.json(room);
    } catch (error) {
  ...
  app.post("/api/bookings", requireAuth, async (req, res) => {
    try {
      ...
      const isExemptRole = user.role === 'hostel_admin' || user.role === 'hostel_team';
      if (!isExemptRole) {
      ...
      const room = await storage.getCommonRoomById(room_id);
      if (!room) {
      ...
      const effectiveBilledTo = (billed_to || (user.role === 'tenant_organization' && user.organization_id ? 'organization' : 'personal')) as any;
      ...
      if (effectiveBilledTo === 'organization' && user.organization_id) {
        ...
        const orgBookings = await storage.getCommonRoomBookings(undefined, user.organization_id);
      ...
      const isAdminOrTeam = user.role === 'hostel_admin' || user.role === 'hostel_team';
      ...
      const booking = await storage.createCommonRoomBooking({
        user_id: user.id,
        room_id,
  ...
  app.get("/api/bookings", requireAuth, async (req, res) => {
    try {
      ...
      if (user.role === "hostel_admin") {
        bookings = await storage.getCommonRoomBookings();
      } 
      // Organization members (both regular and admin) see ALL org bookings + their personal bookings
        ...
        const personalBookings = await storage.getCommonRoomBookings(user.id);
      ...
      // Regular users see only their own bookings
        ...
        bookings = await storage.getCommonRoomBookings(user.id);
      }
  ...
  app.patch("/api/bookings/:id/status", requireAuth, async (req, res) => {
    try {
      ...
      const booking = await storage.updateCommonRoomBookingStatus(id, status);
      res.json(booking);
    } catch (error) {
  ...
  app.patch("/api/bookings/:id/cancel", requireAuth, async (req, res) => {
    try {
      ...
      const booking = await storage.getCommonRoomBookingById(id);
      if (!booking) {
      ...
      // Cancel the booking (no cancelled_by since user is cancelling their own booking)
      const cancelledBooking = await storage.updateCommonRoomBookingStatus(id, 'cancelled');
  ...
  // Admin/Team delete any booking endpoint
  app.delete("/api/admin/bookings/:id", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const adminUserId = (req.user as schema.User).id;
      ...
      const cancelledBooking = await storage.updateCommonRoomBookingStatus(id, 'cancelled', adminUserId);
  ...
  app.post("/api/announcements", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
  ...
  app.patch("/api/announcements/:id", requireAuth, requireRole(["hostel_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.delete("/api/announcements/:id", requireAuth, requireRole(["hostel_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Organization routes
  app.get("/api/organizations", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const { site } = req.query;
  ...
  app.get("/api/organizations/:id/team-count", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
  ...
  app.post("/api/organizations", requireAuth, requireRole(["hostel_admin", "hostel_team"]), (req, res, next) => {
    ...
    try {
      ...
      if (admin_first_name && admin_last_name && admin_email) {
        ...
        const adminUser = await storage.createUser({
          ...
          role: 'tenant_organization_admin',
  ...
  app.get("/api/organizations/:id/employees", requireAuth, async (req, res) => {
    try {
      ...
      if (user.role !== "hostel_admin" && user.organization_id !== orgId) {
  ...
  app.patch("/api/organizations/employees/:id/permissions", requireAuth, requireRole(["member_organization_admin", "hostel_admin"]), async (req, res) => {
    try {
  ...
  app.get("/api/admin/users", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const { site } = req.query;
      // Debug logging removed to reduce compute costs
  ...
  app.post("/api/admin/users", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      // Debug logging removed to reduce compute costs
  ...
  app.patch("/api/admin/users/:id", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const userId = parseInt(req.params.id);
  ...
  app.patch("/api/admin/organizations/:id", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const orgId = req.params.id;
      let updates = req.body;
  ...
  app.delete("/api/admin/users/:id", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const userId = parseInt(req.params.id);
  ...
  app.delete("/api/admin/organizations/:id", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const orgId = req.params.id;
  ...
  app.get("/api/admin/bookings", requireAuth, requireRole(["hostel_admin", "hostel_team"]), async (req, res) => {
    try {
      const { site } = req.query;
      // Get all bookings for admin dashboard with optional site filtering
      const bookings = await storage.getCommonRoomBookings(undefined, undefined, site as string);
      res.json(bookings);
    } catch (error) {
  ...
  app.get("/api/admin/stats", requireAuth, requireRole(["hostel_admin"]), async (req, res) => {
    try {
      const { site } = req.query;
      ...
      const bookings = await db.select().from(schema.common_room_bookings);
      const organizations = await db.select().from(schema.organizations);
  ...
  app.post("/api/organizations/:id/invoice", requireAuth, requireRole(["member_organization_admin", "hostel_admin"]), async (req, res) => {
    try {
      const user = req.user as schema.User;
      const orgId = req.params.id;
      ...
      const bookings = await storage.getCommonRoomBookings(undefined, orgId);
  ...
  app.post("/api/admin/impersonate/:userId", requireAuth, requireRole(["hostel_admin"]), async (req, res) => {
    try {
  ...
  app.get("/api/bookings/pdf", requireAuth, async (req, res) => {
    try {
      ...
      const bookings = await storage.getCommonRoomBookings(userId);
