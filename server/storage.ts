...
export interface IStorage {
  ...
  getCafeOrders(userId?: number, site?: string): Promise<any[]>;
  ...
  // Common Rooms
  getCommonRooms(site?: string): Promise<schema.CommonRoom[]>;
  getCommonRoomById(id: number): Promise<schema.CommonRoom | undefined>;
  createCommonRoom(room: schema.InsertCommonRoom): Promise<schema.CommonRoom>;
  updateCommonRoom(id: number, updates: Partial<schema.CommonRoom>): Promise<schema.CommonRoom>;
  
  // Common Room Bookings
  createCommonRoomBooking(booking: schema.InsertCommonRoomBooking): Promise<schema.CommonRoomBooking>;
  getCommonRoomBookings(userId?: number, site?: string): Promise<any[]>;
  getCommonRoomBookingById(id: number): Promise<any>;
  updateCommonRoomBookingStatus(id: number, status: string, cancelledBy?: number): Promise<schema.CommonRoomBooking>;
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteUser(id: number): Promise<void> {
    ...
    // Delete user's common room bookings
    await db.delete(schema.common_room_bookings).where(eq(schema.common_room_bookings.user_id, id));
  ...
  async deleteOrganization(id: string): Promise<void> {
    ...
    for (const user of orgUsers) {
      ...
      // Delete user's common room bookings
      await db.delete(schema.common_room_bookings).where(eq(schema.common_room_bookings.user_id, user.id));
    }
  ...
  async getCommonRooms(site?: string): Promise<schema.CommonRoom[]> {
    if (site) {
      return await db.select().from(schema.common_rooms)
        .where(and(eq(schema.common_rooms.is_available, true), eq(schema.common_rooms.site, site as any)))
        .orderBy(asc(schema.common_rooms.name));
    } else {
      return await db.select().from(schema.common_rooms)
        .where(eq(schema.common_rooms.is_available, true))
        .orderBy(asc(schema.common_rooms.name));
    }
  }

  async getCommonRoomById(id: number): Promise<schema.CommonRoom | undefined> {
    const [room] = await db.select().from(schema.common_rooms).where(eq(schema.common_rooms.id, id));
    return room;
  }

  async createCommonRoom(room: schema.InsertCommonRoom): Promise<schema.CommonRoom> {
    const [newRoom] = await db.insert(schema.common_rooms).values(room).returning();
    return newRoom;
  }

  async updateCommonRoom(id: number, updates: Partial<schema.CommonRoom>): Promise<schema.CommonRoom> {
    const [updatedRoom] = await db.update(schema.common_rooms).set(updates).where(eq(schema.common_rooms.id, id)).returning();
    return updatedRoom;
  }

  async createCommonRoomBooking(booking: schema.InsertCommonRoomBooking): Promise<schema.CommonRoomBooking> {
    const [newBooking] = await db.insert(schema.common_room_bookings).values(booking).returning();
    return newBooking;
  }
  ...
  async getCommonRoomBookings(userId?: number, site?: string): Promise<any[]> {
    let query = db.select({
      ...
      site: schema.common_room_bookings.site,
      user: {
        id: schema.users.id,
        first_name: schema.users.first_name,
        last_name: schema.users.last_name,
    ...
    if (conditions.length > 0) {
      return await query
        ...
        .orderBy(desc(schema.common_room_bookings.created_at));
    }

    return await query.orderBy(desc(schema.common_room_bookings.created_at));


  }

  async getCommonRoomBookingById(id: number): Promise<any> {
  ...
  async updateCommonRoomBookingStatus(id: number, status: string, cancelledBy?: number): Promise<schema.CommonRoomBooking> {
    ...
    const [updatedBooking] = await db.update(schema.common_room_bookings).set(updateData).where(eq(schema.common_room_bookings.id, id)).returning();
    return updatedBooking;
  }

  async checkRoomAvailability(roomId: number, startTime: Date, endTime: Date): Promise<boolean> {
    const [existing] = await db.select().from(schema.common_room_bookings)
      .where(
        and(
          eq(schema.common_room_bookings.room_id, roomId),
          eq(schema.common_room_bookings.status, "confirmed"),
          sql`${schema.common_room_bookings.start_time} < ${endTime}`,
          sql`${schema.common_room_bookings.end_time} > ${startTime}`
        )
      );
    return !existing;
  }
